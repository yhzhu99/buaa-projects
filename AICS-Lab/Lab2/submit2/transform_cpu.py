...
def run_ori_pb():
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            input_tensor = sess.graph.get_tensor_by_name('X_content:0')     #input is X_content
            output_tensor = sess.graph.get_tensor_by_name('add_37:0')       #output is add_37
...
def run_ori_power_diff_pb():
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            sess.run(tf.global_variables_initializer())
            #
            input_tensor = sess.graph.get_tensor_by_name('X_content:0')     #_content
            input_power_difference_z=sess.graph.get_tensor_by_name('moments_15/PowerDifference_z:0')    #power_difference_z
            output_tensor = sess.graph.get_tensor_by_name('add_37:0')       #add_37

            #powerdifference
            start_time = time.time()
            ret =sess.run(output_tensor, feed_dict={input_tensor:[X],input_power_difference_z:2}) #Z=2
            end_time = time.time()
...
def run_numpy_pb():
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            sess.graph.as_default()
            sess.run(tf.global_variables_initializer()) #ppower_differ_numpyp->power_differencep
            
            input_tensor = sess.graph.get_tensor_by_name('X_content:0')     #X_content
            output_tensor = sess.graph.get_tensor_by_name('add_37:0')       #add_37
            input_diff = sess.graph.get_tensor_by_name('moments_15/PowerDifference:0')  #
            grad_tensor = sess.graph.get_tensor_by_name('moments_15/StopGradient:0')    #
            conv_tensor = sess.graph.get_tensor_by_name('Conv2D_13:0')

            grad = sess.run(grad_tensor,feed_dict={input_tensor:[X]})
            conv = sess.run(conv_tensor,feed_dict={input_tensor:[X]})

            differ = power_diff_numpy(conv,grad,2)

            start_time = time.time()
            ret = sess.run(output_tensor,feed_dict={input_diff:differ,input_tensor:[X]})
            end_time = time.time()
